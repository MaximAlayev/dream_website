...
const EnrollPage = () => {
  ...
  const [level, setLevel] = useState("1"),
        oneLevelChange = ({target:{level}}) => setLevel(level);
  ...
  const onFormSubmit = e => {
    ...
    else
    {
      ...
      Axios.post(`${API_ID}/enroll`, formData, {
        headers: {
    ...
    <div style={pageStyles}>
      ...
      <div style={{height: height}}>
        ...
        <div style={applyWrapper}> 
          <h1 style={headingAccentStyles}> Enroll </h1>
        </div>
        <Form style = {formStyle} noValidate validated={validated} onSubmit={onFormSubmit}>
          ...
          <Form.Group as={Row} className="mb-3" controlId="volLevel" >
            <Form.Label column sm="2">Level</Form.Label>
            <Col sm='10'>
              <Form.Select required type="level" aria-label="level" onChange={(e) => setLevel(e.target.value)}>
                <option value="1">Choose a level</option>
